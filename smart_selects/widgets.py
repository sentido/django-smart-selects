    ...
    def render(self, name, value, attrs=None, choices=()):
        ...
        (function($) {
            ...
            $(document).ready(function(){
                function fill_field(val, init_value){
                    if (!val || val==''){
                        options = '%(empty_label)s' ? '<option value="">%(empty_label)s<'+'/option>' : '';
                        $("#%(id)s").html(options);
                        $("#%(id)s").trigger('change');
                        return;
                    }
                    $.getJSON("%(url)s/"+val+"/", function(j){
                        var options = '%(empty_label)s' ? '<option value="">%(empty_label)s<'+'/option>' : '';
                        for (var i = 0; i < j.length; i++) {
                            options += '<option value="' + j[i].value + '">' + j[i].display + '<'+'/option>';
                        }
                        $("#%(id)s").html(options);
        ...
        """ % {"chainfield":chain_field, "url":url, "id":attrs['id'], 'value': simplejson.dumps(value, ensure_ascii=False), 'auto_choose':auto_choose, 'empty_label': empty_label}
        final_choices = []
